...
### Hero Chapter
The opening pinned-scroll section of the Experience Route, structured as two acts that share the scroll timeline rather than the frame. Act 1 (initial state): the Face-off — both headliners of the next Event with a "VS" mark — plus the press pass and eyebrow. Act 2: the face-off dissolves — the fighter silhouette fades out — while the red slash draws through the now-empty frame and the title ("THE RANKINGS") reveals against the clean background. The act transition is the Parting: each fighter fades out as the slash draws in, then the title chars rise. The slash renders against the bare background once the fighter is gone. The hero background is a plain `light-dark(#fff, #000)` — no radial gradient, no tinted haze. Fighter images are rendered as straight PNGs with no `mix-blend-mode` or filter treatment, so they sit cleanly against the white or dark base. On mobile (no pin) and under reduced motion, the acts stack as two normally-scrolled, fully visible sections in the same narrative order: face-off first, then the title section. Next-event data is server-rendered (loaded in the route loader), so "TBA" only ever appears when an opponent is genuinely unannounced — never as a loading state.
...
**Desktop layout (≥ md breakpoint):** Each row is a centered horizontal composition — `[photo-col] FIGHTER NAME  vs  FIGHTER NAME [photo-col]` — where each photo-col stacks the Corner Thumbnail above the fighter's Country Flag row. The closing corner mirrors the order so flags and thumbnails flank the names symmetrically.

**Mobile layout (< md breakpoint):** Three-column grid — photos anchored on the extreme sides, fighter names stacked vertically in the center column (`FIGHTER A / vs / FIGHTER B`), Country Flag row below each photo. Photos remain visible; the center column is the primary read.
