...
### Card Chapter
A normally-flowing (unpinned) chapter following the Hero Chapter ("THE CARD") that reveals the remaining Bouts of the next Event as a typographic fight-poster ledger — "NAME vs NAME" rows with weight class, grouped under Card Tier dividers, staggered in as they enter the viewport. Each corner carries a Corner Thumbnail; the row's typographic identity is preserved — the photos garnish the ledger, they don't become it. Deliberately unpinned so the scroll journey reads pin → flow → pin (Hero → Card → Divisions), keeping the card skippable at the user's own pace and absorbing variable bout counts. The pin → flow → pin contrast is one of **scale and pacing** (small inline thumbnails in calm flow vs. the large full-screen pinned compositions on either side), not photo vs. no-photo. See ADR 0009.

### Corner Thumbnail
The per-corner fighter image in a Card Chapter row: a sharp **square, no-radius** head/shoulder crop in **full color**, flanking each side of the "NAME vs NAME" matchup (mirrored, echoing the Hero Face-off at row scale). Always a downscale of the full-body source photo — never upscaled. A corner with no photo on file — including a genuinely TBA opponent — shows a neutral fighter silhouette in the same square, so every row keeps an identical photo slot. Rendered with Mantine `Avatar` (`radius={0}`); its silhouette fallback is the missing-photo state. Full color here does not break ADR 0008 (which governs UI palette, not photographic content).
