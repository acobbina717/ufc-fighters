...
### Experience Route
The cinematic, full-screen entry point at `/experience/`. No header. Narrative arc: **event first, rankings as depth** — Hero Chapter → Card Chapter → Weight Class Grid → End State. The Hero hooks with the headline fight; THE CARD fleshes out the full event; the Weight Class Grid delivers the rankings themselves. The pivot from event to rankings is carried by chapter order alone. (The "slash-as-spine" motif from ADR 0011 was removed — see ADR 0012.)

### Hero Chapter
The opening pinned-scroll section of the Experience Route. The upcoming-event hook: the Face-off composition + the slimmed Press Pass. **Photo-only** — the featured fighter holds the frame while the scroll chrome (Press Pass + scroll hint) fades on pinned scroll. No title text in the Hero. On Hero unpin, the central VS glyph Flips into THE CARD's first bout `vs` mark (glyph only — never a scaled photo). Next-event data is server-rendered (loaded in the route loader), so "TBA" only ever appears when an opponent is genuinely unannounced — never as a loading state.

### Face-off
The composition of the Hero Chapter, arranged in poster symmetry: the two headliners of the next Event bottom-anchored in opposite corners (mirrored masks fading toward center), corner-name captions beneath each, and a central "VS". The eyebrow sits top-center; the slimmed Press Pass reflows to a single centered strip near the bottom. Fighters render in full color with the shared Seating Grade to equalize mismatched source photos; the red VS and a gold champion "C" badge are the only accent colors in the frame. Falls back to a single fighter when the opponent is TBA or has no photo. _(Not yet built — issue #60.)_

### Seating Grade
The single shared photo treatment applied identically wherever a fighter photo appears (Hero Face-off, Corner Thumbnails, Weight Class Cards), so mismatched UFC source PNGs read as one shoot: a common contrast/saturation curve, a bottom-fade `mask-image` into the active base, the crop contract `object-fit: cover; object-position: top` (**downscale only, never upscale**), and a mirrored neutral-silhouette fallback in an identical slot. Lives as a shared token/util, not duplicated per module — `seatingGrade.module.css` (issue #59). Survives the ADR 0012 reversal.
